<script lang="ts">
  ...
  console.log();
  let wargearAbilities = $derived(data.datasheetAbilities.filter((v) => v.type === 'Wargear'));
  let datasheetAbilities = $derived(data.datasheetAbilities.filter((v) => v.type === 'Datasheet'));
  let rangedWeapons = $derived(data.datasheetWargear.filter((v) => v.type === 'Ranged'));
  let meleeWeapons = $derived(data.datasheetWargear.filter((v) => v.type === 'Melee'));
</script>
...
<div class="flex flex-col gap-5">
  ...
  <div class="max-w-4xl">
    <table class="w-full table-fixed">
      ...
      <tbody>
        ...
        {#each data.datasheetModels as model}
          <tr>
            <td>
  ...
  <div class="flex flex-row items-center text-neutral-400">
    ...
    <div class="text-xs">{data.datasheetKeywords.map((v) => v.keyword).join(', ')}</div>
    <!-- {#each data.datasheetKeywords as keyword}
          ...
          <tr>
            <td
              ><div class="flex flex-row items-baseline gap-3">
                <div class="border-none">{wargear.name}</div>
                <div class="border-none text-xs text-neutral-300 uppercase">{wargear.description}</div>
              </div></td
            >
  ...
  <div class="flex flex-col gap-2 border-2 p-2">
    ...
    {#each data.datasheetOptions as o}
  ...
  <div class="flex flex-col gap-2 border-2 p-2">
    ...
    {#each wargearAbilities as ability}
      <div class="">
        <span class="pr-1 font-bold underline">{ability.name}:</span><span>{ability.description}</span>
      </div>
    {/each}
  </div>
  <div class="flex flex-col gap-2 border-2 p-2">
    <h2 class="text-xl font-bold">Unit Composition</h2>
    {#each data.datasheetUnitComposition as com}
      <div class="">
        <span>{@html com.description}</span>
      </div>
    {/each}
    <h2 class="text-xl font-bold">Costs</h2>
    {#each data.datasheetModelsCost as mc}
      <div class="">{mc.description} - {mc.cost}pts</div>
    {/each}
  </div>

  <div class="flex flex-col gap-0 border-2 p-2">
    <h2 class="text-xl font-bold">Leader</h2>
    {#each data.datasheetLeader as bodyguard}
      <div class="">
        <span>{@html bodyguard.name}</span>
      </div>
    {/each}
  </div>
</div>
